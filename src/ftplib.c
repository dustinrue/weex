...
/*
   ...
   1999/??/?? commenting out of calling perror()
              by Yuuki NINOMIYA <gm@debian.or.jp>   
   2000/01/09 FTP mode is decided by ftp_passive[host_number]
              instead of FTPLIB_DEFMODE in FtpConnect()
              by Yuuki NINOMIYA <gm@debian.or.jp>   
*/
...
/*
 * I removed specification of `static' to suppress warning.
    ...
    if (typ == FTPLIB_FILE_WRITE)
    {
		...
		printf("short write: passed %d, wrote %d\n", l, c);
			...
			rate=sent_size*100/file_size;
                ...
                printf(_("           : %ld/%ld (%ld%%) [ "),sent_size,file_size,rate);
                for(i=0;i<rate/5;i++){
                    printf("*");
                }
