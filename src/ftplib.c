...
/* $Id: ftplib.c,v 1.1.1.1 2002/08/30 19:24:53 gmkun Exp $ */

/*
   1999/08/13 implementation of FtpClose2() and FtpChmod() 
              by Masashi Itou <webmaster@maitou.gr.jp>
   1999/11/04 implementation of a progress information in FtpXfer()
              by Yuuki NINOMIYA <gm@debian.or.jp>   
*/
...
/*
 * I removed `static' declaration to suppress warning.
 * But the following version and copyright strings live in the binary.
 * Please type: `% strings weex | grep ftplib' :-)
 *
 * 1999/11/06 Yuuki NINOMIYA <gm@debian.or.jp>   
 */
char *version =
...
static int writeline(char *buf, int len, netbuf *nData)
{
	...
	if ((*ubp == '\n') && (lc != '\r'))
	{
		...
		if (w != FTPLIB_BUFSIZ)
		{
		    fprintf(stderr, _("net_write(1) returned %d, errno = %d\n"), w, errno);
		    return(-1);
		}
		nb = 0;
	    }
	...
	if (nb == FTPLIB_BUFSIZ)
	{
	    ...
	    if (w != FTPLIB_BUFSIZ)
	    {
		fprintf(stderr, _("net_write(2) returned %d, errno = %d\n"), w, errno);
		return(-1);
	    }
	    nb = 0;
	}
	...
	if (w != nb)
	{
	    fprintf(stderr, _("net_write(3) returned %d, errno = %d\n"), w, errno);
	    return(-1);
	}
    }
    return len;
...
static int readresp(char c, netbuf *nControl)
{
    ...
    {
	perror(_("Control socket read failed"));
	return 0;
    }
    if (ftplib_debug > 1)
...
GLOBALDEF void FtpInit(void)
{
	...
	fprintf(stderr,_("Network failed to start: %d\n"),err);
#endif
}

/*
 ...
 *
 * return 1 if successful, -2 if PASV fails, 0 or -1 otherwise
 */
    ...
    netbuf **nData)
{
    ...
    switch (FtpOpenPort(nControl, nData, mode, dir)) {
    case 0:
    case -1:
	return 0;
    case -2:
        fprintf(stderr, _("PASV command fails. Try PORT mode.\n"));
        FtpOptions(FTPLIB_CONNMODE, FTPLIB_PASSIVE, nControl);
...

/*
 * FtpClose2 - close a data connection (only logical close)
 */
GLOBALDEF int FtpClose2(netbuf *nData)
{
    netbuf *ctrl;
    if (nData->dir == FTPLIB_WRITE)
    {
 ...
 *
 * return 1 if successful, -1 cannot open local file, 0 otherwise
 */
static int FtpXfer(const char *localfile, const char *path,
	int host_num, netbuf *nControl, int typ, int mode)
{
    int l,c;
    char *dbuf;
    FILE *local = NULL;
	...
	if (local == NULL || stat(localfile, &sbuf) == -1)
	{
	    strncpy(nControl->response, strerror(errno),
                    sizeof(nControl->response));
	    return -1;
    ...
    {
        sent_size = 0;
        file_size = (long)(sbuf.st_size);
	sprintf(digit_temp, "%ld", file_size);
	digit = strlen(digit_temp);
	
        while ((l = fread(dbuf, 1, FTPLIB_BUFSIZ, local)) > 0){
	    if ((c = FtpWrite(dbuf, l, nData)) < l){
		fprintf(stderr, _("short write: passed %d, wrote %d\n"), l, c);
            }
	    if (command_line_option.silent || command_line_option.quiet ||
		command_line_option.suppress_progress ||
		config.silent[host_num] || config.suppress_non_error_messages[host_num] ||
		config.suppress_progress[host_num]) {
		continue;
	    }

            sent_size += c;
            if (sent_size >= FTPLIB_BUFSIZ) {
                progress_flag = 1;
                if (sent_size > 20000000) {
			rate = sent_size/(file_size / 100);
		} else {
			rate = sent_size * 100 / file_size;
		}
                printf(_("           : %*ld/%ld (%3ld%%) [ "), digit, sent_size, file_size, rate);
                for (i = 0; i < rate / 5; i++) {
                    printf("*");
                }
...
GLOBALDEF int FtpNlst(const char *outputfile, const char *path,
	int host_num, netbuf *nControl)
{
    return FtpXfer(outputfile, path, host_num, nControl, FTPLIB_DIR, FTPLIB_ASCII);
}

/*
 ...
 * return 1 if successful, 0 otherwise
 */
GLOBALDEF int FtpDir(const char *outputfile, const char *path, int host_num, netbuf *nControl)
{
    return FtpXfer(outputfile, path, host_num, nControl, FTPLIB_DIR_VERBOSE, FTPLIB_ASCII);
}

/*
...
GLOBALDEF int FtpGet(const char *outputfile, const char *path,
	char mode, int host_num, netbuf *nControl)
{
    return FtpXfer(outputfile, path, host_num, nControl, FTPLIB_FILE_READ, mode);
}

/*
...
GLOBALDEF int FtpPut(const char *inputfile, const char *path, char mode,
	int host_num, netbuf *nControl)
{
    return FtpXfer(inputfile, path, host_num, nControl, FTPLIB_FILE_WRITE, mode);
}

/*
...

/*
 * FtpChmod - chmod a file at remote
 *
 * return 1 if successful, 0 otherwise
 */
GLOBALDEF int FtpChmod(const char *file, const char *mode, netbuf *nControl)
{
    char cmd[256];
