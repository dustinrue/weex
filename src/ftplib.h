...
/* $Id: ftplib.h,v 1.1.1.1 2002/08/30 19:24:53 gmkun Exp $ */
...
#define ftpOpen(x, y) FtpConnect(x, y, &DefaultNetbuf)
...
#define ftpChmod(x, y) FtpChmod(x, y, DefaultNetbuf)
...
GLOBALREF int FtpClose2(netbuf *nData);
...
GLOBALREF int FtpNlst(const char *output, const char *path, int host_num, netbuf *nControl);
GLOBALREF int FtpDir(const char *output, const char *path, int host_num, netbuf *nControl);
